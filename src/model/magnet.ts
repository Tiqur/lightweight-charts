	...
	public align(price: number, index: TimePointIndex, pane: Pane): number {
		...
		const candidates = serieses.reduce(
			(acc: Coordinate[], series: Series) => {
				...
				return acc.concat([ps.priceToCoordinate(bar.value[PlotRowValueIndex.Open], firstPrice.value), ps.priceToCoordinate(bar.value[PlotRowValueIndex.High], firstPrice.value), ps.priceToCoordinate(bar.value[PlotRowValueIndex.Low], firstPrice.value), ps.priceToCoordinate(bar.value[PlotRowValueIndex.Close], firstPrice.value)]);
			},
			[] as Coordinate[]);
